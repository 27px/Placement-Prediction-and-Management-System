...
function skillExists(value) {
    return Array.from(_("#skill-view").children).some(
...
function recheckSkillValues() {
    _("#skills").value = Array.from(_("#skill-view").children)
...
function replaceIdName(t, n, msg, type) {
    ...
    c.setAttribute(
        "onchange",
        `selectedImageOrPdf(this,'view-${type}certificate-${n}',${msg});`
    );
...
function multiFormMove(type, direction) {
    ...
    for (let i = 0, n = x.length; i < n; i++) {
        ...
        if (mf.classList.contains("multiple-active")) {
            var ret;
            if (type == "course") {
                ret = isMultiFormCourseValid(
                    pi(mf.getAttribute("data-multiple"))
                );
                if (ret.success != true) {
...
function setProgress(clock, progress, total, unit) {
    ...
    _("#svg_" + clock + "_path").setAttribute(
        "stroke-dasharray",
        cur + "," + (dash - cur)
    );
...
function addAnotherForm(x, type) {
    ...
    if (n > 0) {
        ...
        for (let i = 0, n = mxf.length; i < n; i++) {
            ...
            if (mf.classList.contains(`multiple-active`)) {
                if (type == "course") {
                    var ret = isMultiFormCourseValid(
                        pi(mf.getAttribute("data-multiple"))
                    );
                    if (ret.success != true) {
                        setMessage("#message-box-8", ret.message, ret.type);
                    ...
                    var ret = isMultiFormExpAchValid(
                        pi(mf.getAttribute("data-multiple")),
                        "experience"
                    );
                    if (
...
function nextPage(event) {
    ...
    if (from == 1 && !_(".pro").children[0].classList.contains("active-box")) {
        setMessage(
            "#message-box-1",
            "Verify OTP before moving to next",
            "warning"
        );
...
function setUpSemester() {
    ...
    var option =
        course.selectedIndex < 0 ? "" : course.options[course.selectedIndex];
...
function selectedProfilePic(element) {
    ...
    if (file != undefined) {
        if (file.type.split("/")[0] != "image") {
            ...
            setMessage(
                "#message-box-2",
                `Add File size < ${max_file_upload_size} MB`,
                "error"
            );
            profilepic.classList.remove("file-selected");
            profilepic.classList.add("file-select-error");
...
function isSecondPageValid() {
    var msg = "#message-box-2",
        name = _("#name").value,
        about = _("#about").value,
        pic = _("#profilepic").files[0];
    if (name == "") {
        ...
        setMessage(
            msg,
            `Add File less that ${max_file_upload_size} MB`,
            "error"
        );
        return false;
    }
...
function isFourthPageValid() {
    var msg = "#message-box-4",
        admnumber = _("#admnumber").value,
        admtype = _("#admtype"),
        admyear_raw = _("#admyear").value,
        idcard = _("#idcard");
    var admyear = pi(admyear_raw);
    admtype =
        admtype.selectedIndex < 0
            ? ""
            : admtype.options[admtype.selectedIndex].value;
    if (admnumber == "") {
        ...
        admyear < new Date().getFullYear() - 10
    ) {
...
function isFifthPageValid() {
    var msg = "#message-box-5",
        course = _("#course"),
        semester = _("#semester"),
        passout = _("#passout").value;
    course =
        course.selectedIndex < 0
            ? ""
            : course.options[course.selectedIndex].value;
    semester =
...
function isSixthPageValid() {
    var msg = "#message-box-6",
        sslcboard = _("#sslcboard"),
        sslcschool = _("#sslcschool").value,
        sslcpercent = _("#sslcpercent").value,
        sslcpassdate = _("#sslcpassdate").value,
        sslccertificate = _("#sslccertificate");
    sslcboard =
        sslcboard.selectedIndex < 0
            ? ""
            : sslcboard.options[sslcboard.selectedIndex].value;
    if (sslcboard == "") {
        ...
        !selectedImageOrPdf(sslccertificate, "view-sslccertificate", 6)
    ) {
...
function isSeventhPageValid() {
    var msg = "#message-box-7",
        plustwoboard = _("#plustwoboard"),
        plustwoschool = _("#plustwoschool").value,
        plustwopercent = _("#plustwopercent").value,
        plustwopassdate = _("#plustwopassdate").value,
        plustwocertificate = _("#plustwocertificate");
    plustwoboard =
        plustwoboard.selectedIndex < 0
            ? ""
            : plustwoboard.options[plustwoboard.selectedIndex].value;
    if (plustwoboard == "") {
        setMessage(msg, "Select board", "error");
        return false;
        ...
        !selectedImageOrPdf(plustwocertificate, "view-plustwocertificate", 7)
    ) {
        setMessage(msg, "Select Certificate", "error");
        return false;
    }
...
function isMultiFormCourseValid(n) {
    var coursetype = _(`#coursetype-${n}`),
        coursename = _(`#coursename-${n}`).value,
        coursecollege = _(`#coursecollege-${n}`).value,
        coursecgpa = _(`#coursecgpa-${n}`).value,
        coursepassdate = _(`#coursepassdate-${n}`).value,
        coursecertificate = _(`#coursecertificate-${n}`);
    coursetype =
        coursetype.selectedIndex < 0
            ? ""
            : coursetype.options[coursetype.selectedIndex].value;
    if (coursetype == "") {
        return {
    ...
    if (coursename == "") {
        ...
        !selectedImageOrPdf(coursecertificate, `view-coursecertificate-${n}`, 8)
    ) {
        return {
...
function isMultiFormExpAchValid(n, type) {
    ...
    xtype =
        xtype.selectedIndex < 0 ? "" : xtype.options[xtype.selectedIndex].value;
    var msg;
    if (type == "experience") {
        msg = 9;
    } else if (type == "achievement") {
