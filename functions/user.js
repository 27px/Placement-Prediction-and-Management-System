...
class User {
    ...
    async initialize() {
        if (
            ...
            var data = {};
            return await this.checkCredentials(cookie.u, cookie.p)
                .then((data) => {
    ...
    hasAccessOf(
        access //type other user types
    ) {
        if (access == "guest") {
            //All users have access previlage of guest
            return true;
        } else if (this.type == "student" && access == "student") {
            return true;
        } else if (
            this.type == "coordinator" &&
            ["student", "coordinator"].includes(access)
        ) {
            //coordinator is also a student
            return true;
        } else if (this.type == "recruiter" && access == "recruiter") {
            //recruiter has no other previlages.
            return true;
        } else if (
            this.type == "admin" &&
            ["coordinator", "admin"].includes(access)
        ) {
            //Admin have coordinator access too
            return true;
        } else {
            return false;
        }
