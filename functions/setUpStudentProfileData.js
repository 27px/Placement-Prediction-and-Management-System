class setUpStudentProfileData {
    constructor(form, files) {
        ...
        for (
            let i = 1;
            form[`coursetype-${i}`] != "" &&
            form[`coursetype-${i}`] != undefined;
            i++
        ) {
            var coursedata = {};
            ["type", "name", "college", "cgpa", "passdate"].forEach(
                (details) => {
        ...
        for (
            let i = 1;
            form[`experiencetype-${i}`] != "" &&
            form[`experiencetype-${i}`] != undefined;
            i++
        ) {
            var experiencedata = {};
            ["type", "title", "description", "from", "to"].forEach(
                (details) => {
        ...
        for (
            let i = 1;
            form[`achievementtype-${i}`] != "" &&
            form[`achievementtype-${i}`] != undefined;
            i++
        ) {
            var achievementdata = {};
            ["type", "title", "description", "from", "to"].forEach(
