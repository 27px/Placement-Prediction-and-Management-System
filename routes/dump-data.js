    ...
    await user
        ...
        .then((cursor) => {
            return Promise.all([
                ...
                fs.readFile(
                    path.join(__dirname, "..", "neural_network", "data.json")
                ),
            ]);
        })
        .then(([result, oldDataSet]) => {
            oldDataSet = JSON.parse(oldDataSet.toString());
            oldDataSet = oldDataSet.concat(
                result.map((student) => {
...
dumpData.post("/dump-user/:user", async (req, res) => {
    ...
    await user
        ...
        .then((data) => {
            ...
            if (
                !(
                    req.params.user == "student" ||
                    req.params.user == "recruiter" ||
                    req.params.user == "coordinator"
                )
            ) {
        ...
        .then((data) => {
            ...
            if (
                ![
                    ...
                    "profilepic",
                    "resource",
                ].includes(req.params.folder)
            ) {
                throw new Error("Invalid Folder");
            }
            return fs.readdir(
                path.join(__dirname, "..", "data", req.params.folder)
            );
        })
        .then((dir) => {
            return Promise.all(
