...
student.get("/profile/new",async(req,res)=>{
  ...
  await user.initialize().then(async(data)=>{
    console.log("otp code commented");
    console.log(data);

    //remove this
    isLoggedIn=data.isLoggedIn;//temporary
    ...
    //     });
    //   });
    //   var transporter=await nodemailer.createTransport(config.MAIL.AUTHENTICATION);
    ...
    //             <i>OTP is valid for <b style="color:#F57F17;">10 minutes</b></i>
    //           </div>
    //         `
    //   }).then(async sent=>{
    //     console.log(sent);
