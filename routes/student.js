...
student.get("/dashboard", async (req, res) => {
    ...
    await user
        ...
        .finally(async () => {
            if (isLoggedIn === true && user.hasAccessOf("student")) {
        ...
        .then(async (data) => {
            ...
            if (isLoggedIn) {
                ...
                if (isLoggedIn && profile != undefined) {
                    ...
                    var rdr =
                        userData.result.type == "coordinator"
                            ? "student"
                            : userData.result.type;
    ...
    await user
        ...
        .finally(async () => {
            if (!isLoggedIn) {
                ...
                const student_data = new setUpStudentProfileData(
                    req.body,
                    req.files
                ).data;
                ...
                var up = [
                    req.files.profilepic.mv(
                        ...
                        }.${req.files.profilepic.name.split(".").pop()}`
                    ),
                    req.files.idcard.mv(
                        `${dir}/idcard/${
                            userData.result.email
                        }.${req.files.idcard.name.split(".").pop()}`
                    ),
                    req.files.sslccertificate.mv(
                        `${cdir}/${
                            userData.result.email
                        }-sslc.${req.files.sslccertificate.name
                            .split(".")
                            .pop()}`
                    ),
                    req.files.plustwocertificate.mv(
                        `${cdir}/${
                            userData.result.email
                        }-plustwo.${req.files.plustwocertificate.name
                            .split(".")
                            .pop()}`
                    ),
                ];
                ["course", "experience", "achievement"].forEach((cert) => {
                    student_data.education[cert].forEach((temp, i) => {
                        up.push(
                            req.files[`${cert}certificate-${i + 1}`].mv(
                                `${cdir}/${userData.result.email}-${cert}-${
                                    i + 1
                                }.${req.files[
                            ...
                            await db
                                ...
                                .updateOne(
                                    ...
                                    },
                                    {
                                        $set: {
                                            data: student_data,
                                            pic_ext: req.files.profilepic.name
                                                .split(".")
                                                .pop(),
                                        },
                                    }
                                ...
                                .then((t) => {
                                    res.redirect(
                                        `/${userData.result.type}/dashboard`
                                    );
                                });
                        });
                    })
